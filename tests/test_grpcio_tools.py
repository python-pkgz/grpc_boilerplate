...
class TestGrpcioTools:
    @contextmanager
    def server(self, hostport: str, tls: bool, tls_crt_path: str = "", tls_key_path: str = "") -> grpc.Server:
    ...
    def test_grpcio_tools(self):
        with self.server("127.0.0.1:50002", False):
            with api_stub("h2c://127.0.0.1:50002", helloworld_pb2_grpc.GreeterStub) as client:
                resp = client.SayHello(helloworld_pb2.HelloRequest(name="World"))
                assert resp.message == "Hello, World"

                resp = client.SayHello(helloworld_pb2.HelloRequest(name="Morld"))
